...
17:01 — hardening — Locked `npm run demo:offline` to rebuild and serve the production candidate at `127.0.0.1:4173` with a strict port; `verify:offline` now rejects remote build references, missing Replay assets, and application network APIs, but it does not replace a physically disconnected browser run.
17:02 — architecture — Added an injectable replay scheduler and five-cycle resource-spy checks: Replay returns to zero timers and fake Preview returns to zero tracks/listeners/attachments after every stop; fake Preview evidence hardens cleanup but cannot reopen the blocked attached-device gate.
17:03 — evidence — Production Replay completed twice with a reload, both hash routes survived reload, and rendered asset references were local; external networking remained connected, so Phase 4’s strict offline and freeze gates stay open.
17:06 — verification — Detached commit `ed19552` passed `npm ci --offline` and the complete hardening suite from a clean worktree; clean-checkout verification is closed, while physical network disconnection, Phase 3 manual evidence, second-builder review, and freeze remain open.
